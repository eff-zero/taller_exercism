...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    if temperature < 800:
        if product_temp_neutrons < 500000 and neutrons_emitted > 500:
            return True
    return False
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percentage_efficiency = (generate_power/theoretical_max_power) * 100
    if percentage_efficiency >= 80:
        return 'green'
    if percentage_efficiency >= 60:
        return 'orange'
    if percentage_efficiency >= 30:
        return 'red'
    return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    higher_limit = threshold + threshold*.10
    lower_limit = threshold - threshold*.10
    temp_by_neutrons = temperature * neutrons_produced_per_second

    if temp_by_neutrons >= lower_limit and temp_by_neutrons <= higher_limit:
        return 'NORMAL'
    if temp_by_neutrons < lower_limit:
        return 'LOW'
    return 'DANGER'
